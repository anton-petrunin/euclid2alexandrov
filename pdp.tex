\chapter{Zalgaller's folding theorem}
...
%Note that since all triangles are acute, we have
%$$V_i\subset S(w_i)$$ for all $i$.
%In particular, for any point $x\in V_i$, there is a unique geodesic $[w_i,x]$,
%which is a line segment in a triangle or an edge of $\mathcal{T}_0$.
...
%\footnote{Here we again use that all the triangles are acute.
%Namely we use that circumcenter of acute triangle lies in its interior.}
...
%(In addition to the sides and vertices of the triangles, $\mathcal{T}_1$  might have white-black edges.) 
...
%From \ref{eq:w-on-edge}, $x$ and $y$ have to lie on different 
%edges of $\mathcal{T}^1$;
...
\addtocontents{toc}{\protect\end{quote}}
\chapter{Gromov's rumpling theorem}\label{sec:S^2->R^2}
\addtocontents{toc}{\protect\begin{quote}}
...
In the proof of the theorem we will use the following exercise. %(see the solution on page \pageref{sol-problem2}). %???
...
\addtocontents{toc}{\protect\end{quote}}
\chapter[Arnold's problem on paper folding]{Arnold's problem\\ on paper folding}\label{sec:arnold}
\addtocontents{toc}{\protect\begin{quote}}
...
\footnote{Here is \textattachfile[color=0 0 1,mimetype=text/html]{pics/napkin.html}{the html-file} which tells how it happened.}.
...
\begin{itemize}
\item \textattachfile[color=0 0 1,author=Alexey Tarasov,mimetype=video/avi]{pics/rouble.avi}{The first movie} shows the folding of whole comb.
\item \textattachfile[color=0 0 1,author=Alexey Tarasov,mimetype=video/avi]{pics/layer-up.avi}{The second movie} shows the folding of one the needles separately.
...
\addtocontents{toc}{\protect\end{quote}}
\chapter*{Final remarks}\label{sec:pdp-comments}
