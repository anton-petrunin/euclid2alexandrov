...
%Assume $f$ distance nonexpanding 
%but not length preserving.
%Then we can choose a curve $\gamma\:[0,1]\to X$ such that 
%$$\length \gamma>\length (f\circ\gamma)+\eps$$
%for some $\eps>0$.
%Applying the definition of length (\ref{def:length}), 
%we can find a partition $0=t_0<t_1<\dots<t_n=1$
%such that
%$$|\gamma(t_0)-\gamma(t_1)|_{X}+\dots+|\gamma(t_{n-1})-\gamma(t_n)|_{X}
%>
%\length \gamma-\tfrac\eps2.$$
%It follows that
%we can take $p=\gamma(t_{i-1})$, $q=\gamma(t_i)$ and
%$\alpha$ to be the arc $\gamma|_{[t_{i-1},t_i]}$ for some $i$.
